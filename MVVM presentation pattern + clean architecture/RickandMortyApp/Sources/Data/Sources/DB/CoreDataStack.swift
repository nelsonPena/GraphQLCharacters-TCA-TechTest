...
@available(iOS 13.0, *)
extension CoreDataStack: CoreDataRepositoryType {
    
    /// Publicador para la lista de los personajes  a eliminar.
    public var savedCharactersPublisher: Published<[CharactersEntity]>.Publisher {
        $characters
    }
    ...
    public func addCharacters(characters: [CharactersEntity]) {
        characters.forEach { characterItem in
            let character = CharactersEntity(context: managedObjectContext)
            character.id = UUID()
            character.characterId = String(characterItem.characterId)
    ...
    public func delete(_ character: CharactersEntity) {
